...
def connect_to_eth():
	url = "https://mainnet.infura.io/v3/78747d4211ba4080840c7c13d4e56776"
...
def is_ordered_block(w3, block_num):
    ...
    def calculate_priority_fee(tx):
        if tx.type == 0 or base_fee == 0:
            return tx.gasPrice - base_fee
        elif tx.type == 2:
            effective_gas_price = min(tx.maxPriorityFeePerGas + base_fee, tx.maxFeePerGas)
            return effective_gas_price - base_fee
        else:
            return 0

    priority_fees = [calculate_priority_fee(tx) for tx in block.transactions]
...
def get_contract_values(contract, admin_address, owner_address):
	...
	prime = contract.functions.primes(owner_address).call()
